"""Render-ready geometry loading for the preview.

Bundles DTZxPorter's Cast binary parser, the SubMesh/bounds helpers shared with mmb_loader,
...
# --------------------------------------------------------------------------
# render-ready geometry extraction + by-extension dispatch
...
def load_cast(path):
    """Render-ready geometry from a .cast model (static rest pose, no skinning)."""
    c = Cast.load(path)
    model = None
    for r in c.rootNodes:
        for ch in r.childNodes:
            if _four(ch.identifier) == "modl":
                model = ch
                break
    if model is None:
        raise ValueError("no model node in cast")

    # material hash -> name
    mat_names = {}
    for ch in model.childNodes:
        if _four(ch.identifier) == "matl":
            n = ch.properties.get("n")
            mat_names[ch.hash] = n.values[0] if n else "?"

    meshes = []
    for ch in model.childNodes:
        if _four(ch.identifier) != "mesh":
            continue
        name = ch.properties["n"].values[0]
        vp = ch.VertexPositionBuffer()
        if not vp:
            continue
        ...
        meshes.append(SubMesh(name, pos, nrm, uv0, idx, mh, uv1=uv1))
