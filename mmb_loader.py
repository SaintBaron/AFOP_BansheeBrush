...
class _R:
    ...
    def seek(self, o, w=0): self.f.seek(o, w)
    def tell(self): return self.f.tell()
    def read(self, n): return self.f.read(n)
    def u8(self):  return self.f.read(1)[0]
    ...
    def int16_norm(self):
        i = self.u16(); v = (i ^ 0x8000) - 0x8000
        return v / 32767.0
    def int8_norm(self):
        i = self.u8(); v = (i ^ 0x80) - 0x80
        return v / 127.0
...
def _parse(buf):
    ...
    if version not in (15, 16, 17):
        raise ValueError(f"unsupported .mmb version {version} (this loader does 15/16/17)")

    bone_count = r.u32()
    for _ in range(bone_count):           # skip skeleton: name + 4x4 matrix + parent
        r.name(); r.seek(64, 1); r.u16()
...
def _parse_mesh(r, version, asset_size):
    ...
    r.seek(1, 1)
    x_count = r.u8(); r.seek(1 + 4 * x_count, 1)
    u_count = r.u16()
    ...
    m.uv_count = r.u8(); r.seek(4 * m.uv_count, 1)
    if version in (16, 17):
        m.color_count = r.u8(); r.seek(4 * m.color_count, 1)
        r.seek(4, 1)
        count_c = r.u8(); r.seek(4 * count_c, 1)
    else:                                  # v15
        r.seek(4, 1)
        m.color_count = r.u8(); r.seek(4 * m.color_count, 1)
...
def _positions(ext, m, lod):
    ...
    region = np.frombuffer(ext, np.uint8, count=n * stride, offset=off).reshape(n, stride)
...
def _uvs(ext, m, lod):
    ...
    n = lod.vertex_count; stride = m.normals_stride
    ...
    region = np.frombuffer(ext, np.uint8, count=n * stride, offset=base).reshape(n, stride)
    u16 = region[:, pre:pre + 4 * nuv].copy().view(np.uint16).reshape(n, 2 * nuv)
    i16 = u16.view(np.int16)
    # compact (12-bit) vs int16_norm detection, tested on the first UV's u component
    compact = not np.any(np.abs(i16[:, 0].astype(np.int32)) > 8191)
    if compact:
        uvf = (u16.astype(np.int64) % 4096).astype(np.float64) / 4095.0
    else:
        uvf = i16.astype(np.float64) / 32767.0
    out = [np.ascontiguousarray(uvf[:, 2 * k:2 * k + 2].astype(np.float32)) for k in range(nuv)]
    uv0 = out[0]
...
def _faces(ext, lod):
    r = _R(ext); r.seek(lod.face_block_offset)
    n = lod.index_count
    use32 = False
    if n > 0:
        if lod.size_a == lod.face_block_offset // 4 and lod.size_a != lod.face_block_offset // 2:
            use32 = True
        else:
    ...
    if use32:
        ...
        idx = np.frombuffer(ext, np.uint16, count=n, offset=lod.face_block_offset).astype(np.uint32)
    return idx.copy()
